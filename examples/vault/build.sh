...
if ! compiler_version="$(rustc --version 2>/dev/null)"; then
    ...
    echo "From the repository root: nix develop --command bash examples/vault/build.sh $mode" >&2
    exit 1
fi
...
built="$temporary/vault.wasm"

# The crate name and stripped metadata keep temporary/source paths out of the
# artifact identity. These flags are part of the committed evaluator identity.
rustc predicate.rs \
    --crate-name sapio_vault_predicate \
    --crate-type cdylib \
    --edition 2021 \
...
if [[ $(wc -c < "$built") -gt 65536 ]]; then
    echo "Vault predicate exceeds the 64 KiB inline program limit" >&2
    exit 1
fi
if [[ "$mode" == --write ]]; then
    install -m 644 "$built" vault.wasm
else
    # A missing or different artifact fails without updating the checked-in file.
    cmp "$built" vault.wasm
fi
