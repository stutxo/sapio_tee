...
pub struct Measurement {
    ...
    pub heap_peak_requested_bytes: Option<u32>,
...
impl MeteredGuest {
    ...
    pub fn measure(&self, parameters: &[u8], view: &[u8], witness: &[u8]) -> Result<Measurement> {
        ...
        // Optional allocator metric, read after the evaluation's fuel/time
        // snapshot. Existing guests have no export and remain uninstrumented.
        let heap_peak_requested_bytes = instance
            .exports
            .get_typed_function::<(), u32>(&store, "checkstwo_heap_peak_requested_bytes")
            .ok()
            .map(|getter| getter.call(&mut store))
            .transpose()
            .context("reading guest heap peak")?;
        Ok(Measurement {
            result,
            fuel: self.fuel - left,
            memory_bytes,
            heap_peak_requested_bytes,
            elapsed,
            phases,
            marker_calls,
        })
