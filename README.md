...
experiments/op_checkzkp/target/release/checkzkp-generic-probe --generate-corpus \
  experiments/op_checkzkp/target/generic-comparison/corpus.json
...
Generation refuses to overwrite an existing corpus. Fixtures use real official
setup/proving for 0/1/6/32-input algebraic workloads, not an authorization policy;
...
### Experiment: non-signing STWO arithmetic

[`experiments/op_checkstwo`](experiments/op_checkstwo) pins STWO 2.3.0 and proves
exactly 1,023 transitions of `(a, b) -> (b, a*a + b*b)` over M31
(`p = 2^31 - 1`). Initial and final states are public, canonical field elements.
The 1,024-row AIR uses a constrained square-sum column to keep all seven
constraints quadratic. Its first/last selector commitment is generated before
the guest build and embedded in the module; neither parameters nor witnesses
can select a different circuit or preprocessing root.

The transcript binds both endpoints, the fixed protocol/PCS configuration,
and `T = SHA256("sapio/checkstwo/public-recurrence/v1" || SHA256(signed_view))`.
This is public-input binding, **not authorization**: anyone can compute and
prove these statements. There is no secret credential, privacy claim, oracle
construction, signing, funding, or deployment. **Do not use this guest as a
spending policy.** The existing Groth16 authorization rule is unchanged.
These measurements are **not a like-for-like comparison** with its SHA256
credential circuit.

The fixed PCS preset uses 64 queries, fourfold blowup, fold step one, a constant
last layer, no proof of work, and lifting log size 12. This records the
configuration, not a reviewed security-level guarantee.

```sh
bash experiments/op_checkstwo/run.sh fetch
bash experiments/op_checkstwo/run.sh all
# Expected nonzero exit: the unchanged production admission gate rejects it.
bash experiments/op_checkstwo/run.sh diagnostic
bash experiments/op_checkstwo/run.sh test
```

The Linux/NixOS runner requires bubblewrap and cached pinned Nix toolchains.
Native proving uses nightly 2026-08-12; the guest uses stable 1.98.1 and the
existing Binaryen 132 optimization pass. After dependency fetching, Rust
builds and proof executions run offline in resource-bounded namespaces with
read-only sources/caches and no host home or credentials. Only the experiment's
ignored `target/` directory is host-writable; disposable scratch is also writable.
The generated selector root must exist before
the guest can compile; there is no placeholder root.

Observed results:

| Measurement | Result | Unchanged limit |
| --- | ---: | ---: |
| Optimized module | 190,826 bytes — rejected | 65,536 bytes |
| `[3, 5]` witness | 63,292 bytes | 65,536 bytes |
| `[0, 0]` witness | 64,032 bytes | 65,536 bytes |
| `[p-1, p-2]` witness | 66,768 bytes — rejected | 65,536 bytes |
| Valid-proof fuel, first two fixtures | 25,862,562 / 26,152,759 | 100,000,000 |
| Peak live requested heap, executed cases | 79,985 bytes | 262,144-byte arena |
| Linear memory, executed cases | 8,388,608 bytes | 67,108,864 bytes |

Fuel and memory figures are **diagnostic only**: the oversized module is
executed with only its module-size gate waived. Fuel, witness, parameter,
function-slot and memory limits remain enforced. The oversized boundary
witness is not executed in WASM. Heap counters measure live requested bytes,
not allocator metadata, fragmentation, or stack; allocator bookkeeping is
included in measured fuel, and the getter runs after the fuel/time snapshot.

All 45 native checks pass, including the boundary fixture. The diagnostic runs
28 WASM cases: two valid acceptances and 26 rejections covering changed public
inputs/context, configuration/root changes, corrupted/truncated/trailing
proofs, noncanonical fields, and malformed views/parameters. Two regression
tests cover an incorrect interior trace with unchanged endpoints and agreement
between row checks, polynomial proving, and public-context binding.

`target/fixtures/measurement.json` records production rejection;
`target/fixtures/diagnostic.json` records per-case measurements and module hash.
The sampled maxima are not universal bounds or Nitro measurements. This
configuration works functionally but **does not fit Sapio admission**; module
size and proof-size headroom, not the observed fuel or heap, are the blockers.
